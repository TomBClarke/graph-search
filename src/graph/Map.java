...
public class Map{
	...
	public void generateGridMap(){
		...
		for(int i = 0; i < nicksGraph.length; i++){
			Node<Point> node = new Node<Point>(new Point(nicksGraph[i][0],nicksGraph[i][1]));
			map.add(node);
		}
		
		for(int i = 0; i < nicksGraph.length; i++){
			for(int j = 2; j < nicksGraph[i].length; j++){
				map.get(i).addSuccessor(map.get(calIndexInList(nicksGraph[i][j], nicksGraph[i][j + 1])));
				j++;
			}
		}
	}
